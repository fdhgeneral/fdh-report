...
# GLOBAL IMPORTS — MUST BE ABOVE ALL CODE
...
def send_email(
    ...
    # Wrap HTML with logo block
    html_with_logo = f"""
    <html>
      <body style="font-family: Arial, sans-serif;">
        <div style="text-align:center; margin-bottom:25px;">
            <img src="cid:fdh_logo" width="180" style="border-radius:6px;">
        </div>
        {html_body}
      </body>
    </html>
    ...
    alt.attach(MIMEText(html_with_logo, "html"))

    # Attach logo as CID image
if logo_path and os.path.isfile(logo_path):
    ...
    with open(logo_file, "rb") as f:
        ...
        logo.add_header("Content-ID", "<fdh_logo>")
        logo.add_header("Content-Disposition", "inline", filename=logo_file.name)
        msg.attach(logo)

    print("[INFO] Logo attached as inline CID image.")

else:
    print(f"[WARN] Logo file not found at {logo_path}, sending without logo.")
