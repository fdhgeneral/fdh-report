...
def merge_stats(sleeper: dict, yahoo_raw: dict) -> dict:
    """
    Merge Sleeper stats.json with Yahoo history (2020–2023 only).
    Only Yahoo owners who also appear in Sleeper are included.
    Yahoo stats are computed from yahoo_history.json seasons/teams.
    """

    # ------------------------------------------------------------
    # 1. Build set of Sleeper manager names (FDH canonical names)
    # ------------------------------------------------------------
    sleeper_names = {m["display_name"] for m in sleeper["managers"]}

    # ------------------------------------------------------------
    # 2. Prepare Yahoo manager accumulator
    # ------------------------------------------------------------
    yahoo_mgrs = {}

    # ------------------------------------------------------------
    # 3. Iterate through Yahoo seasons (only 2020–2023)
    # ------------------------------------------------------------
    for season in yahoo_raw.get("seasons", []):
        year = season.get("season")

        # FIX 1: was indented 3 spaces → IndentationError crash
        if year not in ("2020", "2021", "2022", "2023"):
            continue  # skip non-Yahoo-era seasons
        ...
        for team in season.get("teams", []):
            ...
            # Skip Yahoo-only managers
            if owner not in sleeper_names:
                continue

            # Initialize manager record if first time seen
            if owner not in yahoo_mgrs:
                yahoo_mgrs[owner] = {
                    "display_name": owner,
                    "yahoo_only": True,
                    "seasons": 0,
            ...
            mgr = yahoo_mgrs[owner]

            # ------------------------------------------------------------
            # 4. Accumulate stats for this season
            # FIX 2: section 4 comment was missing (jumped from #3 to #5)
            # FIX 3: stray `# Add stats` comment was unindented/misplaced
            # ------------------------------------------------------------
            mgr["seasons"] += 1
            mgr["wins"]    += team.get("wins", 0)
            mgr["losses"]  += team.get("losses", 0)
            mgr["ties"]    += team.get("ties", 0)
            mgr["pf"]      += team.get("pointsFor", 0.0)
            mgr["pa"]      += team.get("pointsAgainst", 0.0)

            # Championships / runner-ups
            final_rank = team.get("finalRank")
            if final_rank == 1:
                mgr["championships"] += 1
            elif final_rank == 2:
                mgr["runner_ups"] += 1
    ...
    # ------------------------------------------------------------
    # 5. Merge Sleeper + Yahoo
    # ------------------------------------------------------------
    merged = {m["display_name"]: m for m in sleeper["managers"]}

    for name, y in yahoo_mgrs.items():
        if name in merged:
            s = merged[name]
            s["wins"]          += y["wins"]
            s["losses"]        += y["losses"]
            ...
            s["pf"]            += y["pf"]
            s["pa"]            += y["pa"]
            s["championships"] += y["championships"]
            s["seasons"]       += y["seasons"]
            # FIX 5: Sleeper records have no runner_ups key → use .get()
            #         to avoid KeyError on managers without that field
            s["runner_ups"] = s.get("runner_ups", 0) + y.get("runner_ups", 0)
            # FIX 6: goat_score from Yahoo accumulator is always 0
            #         (initialized but never computed from Yahoo data).
            #         Do NOT add it — it would be a no-op and could mask
            #         the Sleeper-derived score if formula changes later.
            # s["goat_score"] += y["goat_score"]  ← removed
        else:
            merged[name] = y

    # ------------------------------------------------------------
    # 6. Stamp total_ aliases on every manager record
    # FIX 4: total_* fields were NEVER written — HOF / GOAT /
    #         standings / records that read total_wins, total_losses,
    #         total_points, total_seasons got nothing back.
    # ------------------------------------------------------------
    managers_out = []
    for m in merged.values():
        ...
        # Recompute win_pct from fully merged totals
...
def hof_table_row(m: dict, rank: int) -> str:
    # Use merged Yahoo + Sleeper totals
    w        = m.get("total_wins", 0)
    l        = m.get("total_losses", 0)
    t        = m.get("total_ties", 0)
    wp       = win_pct_str(w, l, t)

    # Total points (Yahoo + Sleeper)
    pf       = f"{m.get('total_points', 0):,.1f}"

    playoffs = m.get("playoff_apps", 0)
    titles   = m.get("championships", 0)

    # GOAT score based on merged totals
    goat     = m.get("goat_score", 0)
...
<div class="card">
  <div class="card__header">
    <div style="display:flex;align-items:center;justify-content:space-between;">
      <span style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;font-size:1.05rem;color:#fff;">{r['name_a']}</span>
      <span style="font-family:var(--font-sport);font-size:1.3rem;letter-spacing:.1em;color:var(--fdh-red-vivid);">VS</span>
      <span style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;font-size:1.05rem;color:#fff;">{r['name_b']}</span>
    </div>
  </div>
  <div class="card__body">
    <div style="text-align:center;margin-bottom:.75rem;">
      <span style="font-family:var(--font-display);font-size:2rem;color:var(--fdh-red-vivid);font-family:var(--font-sport);letter-spacing:.04em;">{score}</span>
    </div>
...
<div class="rivalry-card">
  <div class="rivalry-card__header">
    <div class="rivalry-card__manager" style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;text-align:left;">{r['name_a']}</div>
    <div class="rivalry-card__vs">VS</div>
    <div class="rivalry-card__manager" style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;text-align:right;">{r['name_b']}</div>
  </div>
...
def gen_rivalries(stats: dict):
    ...
    for r in top10[:6]:
        ...
        top_cards += (
            '<div class="rivalry-card">'
            '<div class="rivalry-card__header">'
            '<div class="rivalry-card__manager" style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;text-align:left;">' + r['a'] + '</div>'
            '<div class="rivalry-card__vs">VS</div>'
            '<div class="rivalry-card__manager" style="font-family:var(--font-sport);text-transform:uppercase;letter-spacing:.08em;text-align:right;">' + r['b'] + '</div>'
            '</div>'
            '<div class="rivalry-card__score">'
...
def merge_stats(sleeper: dict, yahoo_raw: dict) -> dict:
    ...
    Merge Sleeper stats.json with yahoo_history.json into a unified stats object.
    Yahoo managers are enriched with wins/losses/pf/championships/goat_score.
    """

    # Sleeper managers already have full fields
    sleeper_mgrs = {m["display_name"]: m for m in sleeper["managers"]}

    # Yahoo managers need enrichment
    yahoo_mgrs = {}

    for y in yahoo_raw.get("managers", []):
        name = y["display_name"]

        # Yahoo-only managers start with minimal fields
        mgr = {
            "display_name": name,
            "yahoo_only": True,
            "seasons": y.get("seasons", 0),
            "wins": y.get("total_wins", 0),
            "losses": y.get("total_losses", 0),
            "ties": 0,
            "pf": y.get("pf", 0.0),
            "pa": y.get("pa", 0.0),
            "championships": y.get("championships", 0),
            "runner_ups": y.get("runner_ups", 0),
            "latest_team": name,
            "division": "FDH",
            "goat_score": 0,  # computed below
            "weekly_highs": [],
        }

        # Compute GOAT score (same formula Sleeper uses)
        mgr["goat_score"] = (
            mgr["wins"] * 3 +
            mgr["championships"] * 25 +
            mgr["runner_ups"] * 10 +
            mgr["pf"] / 100
        )
    ...
    # Sleeper managers first
    for name, m in sleeper_mgrs.items():
        merged[name] = m

    # Yahoo managers overwrite or append
    for name, m in yahoo_mgrs.items():
        if name in merged:
            # If a manager appears in both systems, combine stats
            sm = merged[name]
            sm["wins"] += m["wins"]
            sm["losses"] += m["losses"]
            sm["pf"] += m["pf"]
            sm["pa"] += m["pa"]
            sm["championships"] += m["championships"]
            sm["runner_ups"] += m["runner_ups"]
            sm["seasons"] += m["seasons"]
            sm["goat_score"] += m["goat_score"]
        else:
            merged[name] = m
